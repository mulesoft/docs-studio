...
|xref:3.9@mule-runtime::web-service-consumer-example.adoc[Web Service Consumer] |Turns JSON requests to XML and back. Also turns a flow variable into an outbound property.
|xref:3.9@mule-runtime::salesforce-to-database-example.adoc[Salesforce to Database] |Renames fields in a Salesforce record selector. See the "Batch Step 1" section.
|xref:3.9@mule-runtime::service-orchestration-and-choice-routing-example.adoc[Service Orchestration and Choice Routing] |Transforms a message to communicate with the Samsung web service API in the "samsungOrder Flow" section.
