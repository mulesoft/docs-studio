...
This is the same as using the `-DattachMuleSources` flag when packaging using the Mule Maven Plugin, such as in a command-line using the command:
...
Additionally, you can uncheck *Include project modules and dependenciess* to skip bundling the actual modules and external dependencies required to run the Mule application in a Mule runtime. 
In this case a lightweight JAR file package of the Mule application is created that does *not* include any depenedencies specified in the Mule applicataion's `pom.xml` file. This JAR file is *not* a functional deployable archive file and *cannot* be deployed to a Mule runtime, but instead offers a way to archive just the source files that make up the Mule application. This is the same as using the `-lightWeightPackage` flag when packaging using the Mule Maven plugin, such as in a command-line using the command:
...
. See xref:mule-runtime::mmp-concept.adoc#mule-application-packaging[Mule Application Packaging] for more information.
+
