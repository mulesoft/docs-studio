...
[WARNING]
*Important*: if you use the Maven *studio:studio* goal, you cannot use the *m2eclipse* plugin, because it will override the studio:studio goal. Anypoint Studio has built in capabilities for importing and exporting between Maven and Studio using the studio:studio goal, which covers most use cases.

== Assumptions

This document assumes that you have xref:3.7@mule-runtime::downloading-and-starting-mule-esb.adoc[downloaded and installed Anypoint Studio]. If you do not have any previous experience with Eclipse or an Eclipse-based IDE, you can review the brief introduction to the xref:index.adoc[Anypoint Studio interface]. The document also assumes you're familiar with http://maven.apache.org/[Apache Maven].
