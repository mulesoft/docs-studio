...
[WARNING]
*Important*: if you use the Maven *studio:studio* goal, you cannot use the *m2eclipse* plugin, because it overrides the studio:studio goal. Anypoint Studio has built in capabilities for importing and exporting between Maven and Studio using the studio:studio goal, which covers most use cases.

== Prerequisites

This document assumes that you have xref:3.9@mule-runtime::downloading-and-starting-mule-esb.adoc[downloaded and installed Anypoint Studio]. If you do not have any previous experience with Eclipse or an Eclipse-based IDE, please review the brief introduction to the xref:index.adoc[Anypoint Studio interface]. The document also assumes you're familiar with https://maven.apache.org/[Apache Maven].
...
+
image::import-maven.png["A list of import sources with an arrow pointing to a Maven-based Mule project from pom.xml"]
+
...
+
image::importmaven1.png["A dialog box for importing a Maven-based Mule project"]
+
...
image::error-message.png["A table is presented that lists 15 errors, all related to missing required libraries"]
...
+
image::build-path-1.png["A dialog box displaying JARs and class folders on a build path"]
+
. Click *Add Variable...*.
. In the *New Variable Classpath Entry* panel, click the *Configure Variables...* button.
+
image::new-variables-2.png["Dialog box for selecting variables to build a classpath entry"]
+
...
+
image::m2-repo-2.png["A dialog box with classpath variables defined"]
+
. Change the contents of the *Path* field to specify the location of your Maven (`.m2`) repository.
+
image::edit-variable-entry.png["A dialog box for editing variable entry with fields for name and path"]
+
...
* Learn how to build a complete xref:building-a-mule-application-with-maven-in-studio.adoc[Studio Maven Project], including tests and dependencies.
* Learn how to xref:enabling-maven-support-for-a-studio-project.adoc["mavenize" an existing Mule project].
