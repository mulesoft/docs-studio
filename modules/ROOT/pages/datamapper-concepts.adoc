...
image::dw-migrator-script.png["A comprehensive menu offering migration, running, debugging, and other project operations"]
...
+
image::step1.jpg["A window for creating a mapping from CSV to JSON"]
+
...
+
image::step2.jpg["A software interface displaying a data mapping tool with input and output fields"]
...
image::datasense-explorer2.png["A list of filter options, with a search bar and a refresh button"]
...
+
image::flow.png["A flow chart with three nodes: HTTP, Logger, and Set Payload, and an error handling path"]
...
+
image::twitter-configuration.jpg["A form with fields to configure a Twitter connection"]
...
+
image::metadata.jpg["A comprehensive list of status data, including location, time, and user details"]
+
...
+
image::input-and-output.jpg["A data mapping tool with the capability to create a mapping from a list of statuses to a CSV file"]
...
+
image::custom-csv.jpg["A window for defining a CSV file with fields for name, type, and children elements"]
...
+
image::mapping.jpg["A screenshot of a program mapping attributes from a data source to an output file"]
+
...
image::a1-adv-ex.png["A program interface for mapping input and output attributes and elements"]
...
image::1-adv-ex.png["A visual representation of the data mapping process between XML and JSON formats"]
...
image::2-adv-ex.png["Mapping tool to visualize the relationship between XML and JSON data structures"]

image::3-adv-ex.png["A data mapping tool with input and output arguments"]
...
image::filter.jpg["A window with options for configuring XML input"]
