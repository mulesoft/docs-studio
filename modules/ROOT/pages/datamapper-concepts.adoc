...
+
image::input-and-output.jpg["A data mapping tool that converts a map of a status list into a CSV file"]
...
+
image::custom-csv.jpg["A window for defining a CSV file with field for name, type, and child elements"]
...
+
image::mapping.jpg["A screenshot of a program that maps attributes from a data source to an output file"]
+
