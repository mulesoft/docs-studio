...
Using API Sync, Anypoint Studio enables you to import a RAML or OAS 2.0 API specification into either a new Mule project or an existing one.

If you are using Mule runtime engine (Mule) 4.1.3 and earlier, or if you prefer to import an API specification that's not in Exchange, follow the steps presented in xref:studio::import-api-specification-design-center.adoc[import an API specification from a local file] or xref:import-api-secification-design-cetner[import an API specification from Design Center] depending on your prefered use case.
...
After you import them, you can manage the API specifications in your project that are linked to Exchange from the API project management view by clicking the _Manage Mule Project APIs_ icon (image:manage-project-api-icon.png[2%,2%]) in the top taskbar.
