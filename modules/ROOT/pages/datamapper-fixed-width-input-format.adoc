...
:page-aliases: 6.5.x@studio::datamapper-fixed-width-input-format.adoc
:keywords: datamapper
...
The following image illustrates a very simple Mule flow which includes a DataMapper transformer.  The first building block, a xref:3.8@mule-runtime::file-connector.adoc[File Connector], reads the fixed-width input file. Then, DataMapper transforms the data into the JSON format, and outputs the result into a second file.
