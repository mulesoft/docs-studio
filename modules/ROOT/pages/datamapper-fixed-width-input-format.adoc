...
The following image illustrates a very simple Mule flow which includes a DataMapper transformer.  The first building block, a xref:3.7@mule-runtime::file-connector.adoc[File Connector], reads the fixed-width input file. Then, DataMapper transforms the data into the JSON format, and outputs the result into a second file.
...
+
image::dm-editor.png[DM_editor]

. In the *Input* section, click the Edit icon
image:edit-icon.png[edit_icon] next to the *Type* input field, then select *Fixed Width* from the *Type* drop-down menu. The properties editor *Input* pane displays the input options for the fixed-width format, as shown below.
+
image::3-fixedw-options.png[3.fixedw_options]
...
+
image::fixed-width-input.png[fixed_width_input]
...
+
image::4-dm-mapedit.png[4.DM_mapedit]
...
+
image::6-add-field.png[6.add_field]
...
. In the *Output* pane, click the *Infer Metadata* icon image:infer-metadata.png[infer_metadata] , then select **Re-Create Metadata From Input**.
+
image::8-recreate-md.png[8.recreate_md]

DataMapper will automatically recreate the output metadata and display the new mapping, as shown below.

image::9-new-mapping.png[9.new_mapping]
