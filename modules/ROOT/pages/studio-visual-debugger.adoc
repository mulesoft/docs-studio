...
:page-aliases: 6.5.x@studio::studio-visual-debugger.adoc
...
The left pane displays the message payload (in this case, an HTTP GET request received via an xref:3.8@mule-runtime::http-connector.adoc[HTTP Connector]). The right pane displays various metadata associated with the message:
...
A conditional breakpoint is activated only if a user-defined condition evaluates to _true_ when Studio executes the building block with the breakpoint. You define the condition using a xref:3.8@mule-runtime::mule-expression-language-mel.adoc[Mule Expression].
...
* Remote debugging does not work with xref:3.8@mule-runtime::mule-high-availability-ha-clusters.adoc[clusters].
====
...
Before accessing and debugging your application on a remote server, you must first xref:3.8@mule-management-console::deploying-applications.adoc[export and deploy] your application to the desired Mule server.
...
* Learn more about setting xref:3.8@mule-runtime::business-events.adoc[Business Events] in your Enterprise Edition Studio application.
