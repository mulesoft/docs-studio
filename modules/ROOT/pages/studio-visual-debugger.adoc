...
The left pane displays the message payload (in this case, an HTTP GET request received via an xref:3.7@mule-runtime::http-connector.adoc[HTTP Connector]). The right pane displays various metadata associated with the message:
...
A conditional breakpoint is activated only if a user-defined condition evaluates to _true_ when Studio executes the building block with the breakpoint. You define the condition using a xref:3.7@mule-runtime::mule-expression-language-mel.adoc[Mule Expression].
