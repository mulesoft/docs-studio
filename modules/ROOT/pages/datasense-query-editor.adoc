...
The *Query Editor* in Studio allows you to easily query and retrieve complex data structures that you can use when designing your application. Using xref:datasense.adoc[DataSense] on an Anypoint Connector which supports query functionality, the Query Editor can automatically acquire metadata from remote application, saving you the effort of manually discovering which data types and fields are available to query. The Query Editor displays the available data types and fields in a graphical menu – all you need to do is select the ones you want to query.
...
[TIP]
====
Using the Query Editor in conjunction DataSense's xref:using-perceptive-flow-design.adoc[perceptive flow design] greatly facilitates designing and building a SaaS integration flow. Perceptive flow design allows you to create complex mappings by simply placing an xref:3.8@mule-runtime::dataweave.adoc[DataWeave transformer] before or after a connector in your flow.
====

== Assumptions

This document assumes that you are familiar with the xref:index.adoc[Visual Editor in Anypoint Studio], as well as the xref:3.8@mule-runtime::anypoint-connectors.adoc[Anypoint Connectors], in particular the Salesforce cloud connector used in the example queries. This document also references xref:3.8@mule-runtime::anypoint-connectors.adoc[Connectors], xref:3.8@mule-runtime::global-elements.adoc[Global Elements], and xref:datasense.adoc[DataSense]. To learn more about developing with Mule ESB's graphical user interface, review xref:index.adoc[Getting Started with Anypoint Studio].

== Using the Query Editor

The table below lists the xref:3.7@mule-runtime::anypoint-connectors.adoc[Anypoint Connectors] which support Query Editor functionality. (Additionally, you can use the Query Editor with any custom-built connector which supports the query functionality.)
...
. To access the Query Editor, add a connector to your flow, then access its xref:3.7@mule-runtime::the-properties-editor.adoc[Properties Editor.]
...
. Create a Salesforce xref:3.7@mule-runtime::global-elements.adoc[Global Element].
...
* Learn more about the Query Editor's xref:datasense-query-language.adoc[DataSense Query Language].

* Learn more about xref:datasense.adoc[DataSense].