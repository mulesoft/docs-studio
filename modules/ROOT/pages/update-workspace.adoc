...
Anypoint Studio helps you ensure that you are using the latest Mule Maven plugin and that your project points to the latest Anypoint Exchange. When Studio runs in a new workspace for the first time, it validates that every project in your workspace has both dependencies updated.
...
<1> Studio lists all the open projects in your workspace that must add the updated Exchange dependency.
<1> Selecting *Perform update* enables Studio to update the dependencies in the `pom.xml` file of the projects you selected.

[[update-7.1-workspace]]
== Mule Maven Plugin Update for Workspaces Created With Studio 7.1.x and Earlier Versions

Studio simplifies the configuration of the `mule-artifact.json` file. When packaging your Mule application, this file is used to identify the application's classloader configuration and settings, and the minimum required Mule runtime engine version required to run your application.
...
When you open a project created with earlier versions than 7.12, Studio adds this new dependency to your project pom.xml file:
...
You must add this new dependency to take advantage of the latest publication process to Exchange.
