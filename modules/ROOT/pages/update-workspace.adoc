...
Anypoint Studio keeps your workspace's configuration files updated to use the latest Mule Maven plugin version available.
...
The plugin inspects the metadata declared in `mule-artifact.json`. If there is a value that is not declared, the plugin infers it from the application code before packaging. If another metadata is declared, the plugin uses that information to package the application.

If you update to the latest plugin and chose to keep your old descriptor files, you'll need to manually monitor your descriptor metadata.
