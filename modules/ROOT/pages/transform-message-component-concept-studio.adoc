...
image::transform-message-component.png["A code snippet that demonstrates the mapping of data from one object to another"]
...
+
image::show-graphics-mode.png["A code block that displays JSON objects for buyer data."]
...
+
image::source-with-trees-mode.png["Here is a code snippet demonstrating how to transform a payload object into a new object with specific buyer data"]
* *Source Only* mode. +
The Graphical view shows only the input data structure along with the Source Code view.
+
image::source-code-mode.png["A code block displaying a payload object with buyer details"]
...
+
image::editor-mode-preference.png["A menu displaying DataWeave preferences"]
...
* To filter what you see from the input or output structures, click the magnifying glass icon image:tips-transform-message-ui-studio-69e11.png["A magnifying glass icon"] at the top of either section and then start typing the name of a field.
