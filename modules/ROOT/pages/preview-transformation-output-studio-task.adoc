...
+
image::dataweave-edit-sample-data.png["A dropdown menu with options to set metadata, clear metadata, edit sample data, and configure the reader"]
...
+
image::dw-populate-sample-data.png["A code editor displaying a JSON file with fields for item amount, payment, product, salesperson, and shop"]

+
[TIP]
To reset the sample to the original scaffolding with placeholders, click the *rescaffold button* image:dw-rescaffold-button.png["A menu icon with three horizontal lines and a right arrow"]. Note that any sample data you provided is lost.
