...
To disable the breakpoint, right-click the message processor again and select *Disable Breakpoint*:

image::disable-breakpoint.png[]

Though disabled, Studio does not delete the breakpoint. In the canvas, the breakpoint's corresponding event processor displays a gray circle instead of a red circle.

image::disabled-breakpoint.png[]

When you have many breakpoints, you can use the *Mule Breakpoints* view in the *Mule Debug* perspective. The *Mule Breakpoints* view lists all the message processors that have breakpoints and indicates with a check mark all the breakpoints that are enabled. You can check and uncheck to enable or disable the breakpoints you need:

image::breakpoint-activation.png[]

When you run your application in debug mode, Studio stops the flow execution at the breakpoint you added, allowing you to inspect the message's content in the *Mule Debugger* view.
...
+
image::set-confitional-breakpoint.png[]
...
Enabling error breakpoints causes any event processor in the application to become a breakpoint if it throws an error. Studio stops the flow execution whenever an error occurs. The *Mule Debugger* view will display the content of the message as it exists at that point in the flow where the error occured.
...
You can activate the *Mute Breakpoints* option in the *Mule Breakpoints* view, to temporarily disable the debugger to stop at your enabled breakpoints.

image::mute-breakpoints.png[]

By defautl, muting your breakpoints also disables the error breakpoints. You can configure this behavior in *Preferences* > *Anypoint Studio* > *Debugger* and deselcting the *Mute Breakpoints disables Error Breakpoints* check mark:
