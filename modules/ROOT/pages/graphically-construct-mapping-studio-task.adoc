...
image::using-dataweave-in-studio-8e6e0.png["A diagram illustrates the process of converting a JSON payload to a string output"]
...
image::using-dataweave-in-studio-6b171.png["A diagram that displays the data fields for items and their respective data types"]

== Set a Fixed Value or Function

. Double click on a field in the output tree. This adds a function icon image:using-dataweave-in-studio-66cdc.png["A  dot next to the letters f and x"] next to it.
...
+
image::dw-click.png["Code displaying a null text value within a JSON object"]
+
You can add:
