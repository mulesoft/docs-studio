...
:page-aliases: 6.5.x@studio::using-perceptive-flow-design.adoc
:keywords: datamapper

Building upon the certainty that connection testing establishes, Mule takes one more step to make designing your application easier with *Perceptive Flow Design*. If you model a flow to include an Anypoint Connector, preceded or followed by a xref:3.8@mule-runtime::dataweave.adoc[Transform Message component], Mule uses the working connection to the resource to retrieve metadata about message payload and properties. Capturing the data type and structure that a resource employs, Mule feeds that data into the xref:3.8@mule-runtime::dataweave.adoc[Transform Message component] thereby informing the component of the expected input or output.  This functionality facilitates mapping of data from one data format and structure to another.

== Prerequisites

This document assumes that you are familiar with the xref:index.adoc[Visual Editor in Anypoint Studio] and the xref:3.8@mule-runtime::anypoint-connectors.adoc[Anypoint Connectors] and xref:3.8@mule-runtime::dataweave.adoc[DataWeave] that you can use to create applications in Studio.  Further, this document references https://anypoint.mulesoft.com/exchange/?type=connector[Connectors] and xref:3.8@mule-runtime::global-elements.adoc[Global Elements.] Review xref:datasense.adoc[DataSense] to learn more about the overall DataSense functionality.
...
. Once you have confirmed successful connection to Salesforce, insert a xref:3.8@mule-runtime::dataweave.adoc[Transform Message Component] before the connector on your canvas.
+
...
. Drag and drop input fields across to the output fields to map and transform data format to Salesforce-friendly values. Note how the xref:3.8@mule-runtime::dataweave-language-introduction.adoc[DataWeave Code] on the right is written automatically to match every field you drag along.
+
image::dw-perceptive-flow.png[dw_perceptive_flow]
+
. Save, then continue to build your Mule application.
+
[TIP]
See xref:3.8@mule-runtime::dataweave-quickstart.adoc[DataWeave Quickstart Guide] for more complete examples using the same features.
...
* Learn more about xref:3.8@mule-runtime::dataweave.adoc[DataWeave]
