...
:page-aliases: 6.5.x@studio::building-a-mapping-flow-in-the-graphical-mapping-editor.adoc
:keywords: datamapper
...
The _employee_ and _manager_ collections of elements are bold to indicate that they can be mapped to the output pane. Selecting the element mapping "Foreach 'manager' -> 'contact'" shows a different set of mappable elements:  image:2.png[]
...
* When closed, image:eye-open.png[eye_open] hides all elements in the input pane that are not mappable in the currently displayed mapping level.
...
Arguments can reference any information obtainable through Mule Expression Language (MEL), including message and header properties,  filenames, etc. Consult xref:3.8@mule-runtime::non-mel-expressions-configuration-reference.adoc[Non-MEL Expressions Configuration Reference]  for details.
