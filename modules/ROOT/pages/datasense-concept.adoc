...
:page-aliases: datasense-perceptive-flow-design-concept.adoc
...
Consider a flow in which you include an Anypoint Connector preceded or followed by a Transform Message component. The Mule runtime engine uses the working connection to the resource to retrieve and supply metadata about the payload and properties to DataWeave, saving you the effort of manually inspecting Mule event structure.
