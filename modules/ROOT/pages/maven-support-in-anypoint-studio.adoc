...
* Learn more about xref:importing-a-maven-project-into-studio.adoc[importing Maven projects into Studio] or link:/mule-user-guide/v/3.7/enabling-maven-support-for-a-studio-project["mavenizing" existing Studio projects].
* Using Maven outside of Studio? Refer to these instructions for xref:3.7@mule-runtime::configuring-maven-to-work-with-mule-esb.adoc[configuring your Maven installation] to point to the necessary repositories.