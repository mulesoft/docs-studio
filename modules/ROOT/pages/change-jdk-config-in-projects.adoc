...
Studio supports FIPS 140-2 security mode for Java 8, 11, and 17.
...
----
+
. Register the security providers in the `java.security` properties file, located in your JDK 8 `.../lib/security/` folder. Add the Bouncy Castle providers to the beginning of the provider list and renumber the subsequent entries.
+
[source,java]
----
#
...
----
+
. Run your application. The startup readout shows FIPS security mode enabled.
+
[source,text]
----
...
----
[calloutlist]
.. FIPS 140-2 security mode is enabled.
...
[source,bash]
----
cp bc-fips-1.0.2.4.jar bctls-fips-1.0.19.jar bcpkix-fips-1.0.7.jar /Applications/AnypointStudio.app/Contents/Eclipse/plugins/org.mule.tooling.server.4.4.0.ee_7.3.5.202107221543/mule/lib/boot
----
. In Studio, go to your project, right click on its folder, and select *Run configuration*.
. Go to the *Arguments* tab. In *VM arguments*, add:
+
[source,text]
----
-Dmule.security.model=fips140-2 --add-opens=java.base/sun.security.provider=org.bouncycastle.fips.core -Dorg.bouncycastle.fips.approved_only=true
----
. Run your application. The startup readout shows FIPS security mode enabled.
+
[source,text]
