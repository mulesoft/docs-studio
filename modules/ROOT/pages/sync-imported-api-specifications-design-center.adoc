...
Before you begin, ensure that you have the necessary permissions to edit the API specification, and that you don't already have an API specification project for that API specification in your Studio workspace.

. In the Package Explorer view, right-click the API dependency of your API specification project, select the name of the API you want to edit, and select *Import API Spec from MuleSoft VCS*.
+
image::import-dc-working-spec.png["A menu with options, pointing to the Import API Spec from MuleSoft VCS option"]
+
By default, Studio pulls the `main` branch in MuleSoft VCS. If the API version that you imported from Exchange belongs to a different branch in MuleSoft VCS, you must switch to that branch before continuing.
. Apply the changes to your API specification.
. Synchronize your changes with MuleSoft VCS before publishing to Exchange. 
+
...
+
image::update-asset-version.png["A window showing an API diagram, project files, and a menu with options including "Import API Spec from MuleSoft VCS" and "Update Version""]
. Click *Finish*.
