= Defining a Data Mapping Flow with Input and Output Metadata (Deprecated - DataMapper)
...
This document assumes the reader is familiar with the Anypoint DataMapper Transformer. If you are not, start from the beginning: xref:datamapper-user-guide-and-reference.adoc[DataMapper User Guide and Reference]. For a listing of all available tools in Anypoint DataMapper, consult xref:datamapper-visual-reference.adoc[DataMapper Visual Reference].
...
image::basic.png["A form with input and output fields for a data mapper"]
...
image::source2.png["A form with input and output fields to configure a DataMapper"]
...
image::csvex.png["A window that displays options for selecting a data source and editing its structure"]
...
image::definefields.png["A form for defining a CSV file, complete with fields for name, delimiter, and field types"]
...
image::image2013-4-10-1-58-7.png["A drop-down menu that lists various delimiters"]
...
image::image2013-4-10-2-46-50.png["A dropdown menu with different delimiter options, selected is "John~Doe~10""]
...
image::connector.png["Workday Connector, a button labeled "Install" and another labeled "View details""]
...
image::fwidth.png["A form with options for data source, type, and structure"]
...
image::image2013-5-7-0-20-33.png["A table displaying data types and widths for employees"]

Note: See xref:datamapper-fixed-width-input-format.adoc[Fixed-Width Input Format] for several examples of working with fixed-width input format data.
...
+
image::json.png["A menu with options for creating a JSON sample from an example, output, or user-defined input"]
...
image::image2013-5-7-0-39-24.png["A structure for defining JSON components, encompassing name, type, and child elements"]
...
* By copying the structure you defined in the input/output
+
image::mapkv.png["A dialog box with options for creating a map sample"]
...
+
image::excel.png["A form with fields for source, type, Excel example, and name"]
...
image::pojo.png["A form with fields for source, type, and class"]
...
+
image::image2013-5-7-1-59-17.png["A dialog box labeled Object Introspector, with a dropdown menu for data structure, and "Cancel" and "OK" buttons"]

Once you have a complete description of the desired class structure, click *OK*.

image::pojo2.png["A form with fields for source, type, and class"]
...
+
image::xml-new-input.png["A form with options to define an XML input source"]
...
+
image::image2013-5-7-1-54-33.png["A window with a list of names and types"]
+
...
Once you finish creating the mapping, the mapping flow definition opens in the graphical mapping editor. At that point you can define the element mappings and transformations for your data, as described in xref:building-a-mapping-flow-in-the-graphical-mapping-editor.adoc[Building a Mapping Flow in the Graphical Mapping Editor].
