...
Handle complex data mappings and transformations through the xref:using-dataweave-in-studio.adoc[*Transform Message*] component, that leverages the powerful xref:3.7@mule-runtime::dataweave.adoc[DataWeave] language. This component allows you to create your transformations both through a simple drag and drop visual interface or by coding in DataWeave to unleash all of the language's potential.
...
Use https://docs.archive.mulesoft.com/munit/v/1.1/using-munit-in-anypoint-studio[MUnit] to automatically create and run unit tests on your applications. That way you can be certain that any changes you make to your application won't brake it or alter how it behaves.

== RAML Support

https://raml.org/[RAML] is a widely used standard for defining APIs through a very simple syntax, similar to that of YAML.

* You can import RAML files for your APIs that are managed in the Anypoint Platform by the xref:api-manager::index.adoc[API Manager].
...
* xref:general:getting-started:build-a-hello-world-application.adoc[Build an application in Anypoint Studio].
