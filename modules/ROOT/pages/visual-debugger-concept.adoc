...
With Anypoint Studio debugger, you can run your application from Studio in debug mode and configure breakpoints to stop the execution of your application at a specific message processor. In the *Mule Debugger* view, inspect the content of the message as it exists at that point in the flow and evaluate a DataWeave expression against it.
...
Additionally, run your application in debug mode by right-clicking in the canvas and selecting *Debug Project*, or by clicking the *Debug* button on the toolbar image:to-run-debug-mode-e7b7b.png[debug-button].
...
| Studio debugger listens to incoming TCP connections on `localhost` and port `6666`. Pause the debugger listening activities by selecting *Suspend* to prevent the debugger from triggering or processing any incoming requests temporarily. 
| 
