...
Manage the API specifications linked to Exchange in your project from the API specification project management view by clicking the *Manage Mule Project APIs* icon (image:manage-project-api-icon.png["A stylized letter with a curved roof over it" 2%,2%]) in the task bar.
...
* When a spec is imported, it's treated as a modification of an existing spec and evaluated for rescaffolding
