...
:page-aliases: 6.5@studio::updating-metadata-in-an-existing-mapping.adoc
:keywords: datamapper

[NOTE]
====
DataMapper will continue to be fully supported in all current and future versions of Mule runtime 3.x, however it will be removed in Mule runtime 4.0 in favour of the *Transform Message* component (based on xref:3.9@mule-runtime::dataweave.adoc[DataWeave] code). We recommend that if you wish to take advantage of the new capabilities of DataWeave or if you start new projects, upgrade now.
...
image::image043.png["A list of fields that can be used to infer metadata, including Name, Last Name, Street, and Zipcode"]
...
image::image049.png["A dialog box with options to generate an XML schema from an example or a user-defined schema"]
...
image::image046.png["A diagram illustrating the transfer of data from a contacts CSV file to a users XML file"]
...
+
image::image047.png["A dropdown menu with options to reload, re-create, or re-create from output metadata"]

The Input pane in the DataMapper will now display the  `Phone` field, as shown below.

image::image048.png["A diagram illustrating the process of transferring data from a CSV to an XML file"]
...
image::image050.png["A diagram illustrating the transformation of data from CSV to XML"]
