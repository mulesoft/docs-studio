...
+
image::index-039ff.png["A flowchart showing a listener, payload, and logger"]
+
...
+
image::index-a2191.png["A table that lists global configuration elements, including type, name, and description, with buttons to create, edit, and delete them"]
+
...
+
image::index-ebe7d.png["XML code for a Mule project"]
+
...
image::index-ee160.png["A file tree showing a project's structure and dependencies"]
...
image::mule-palette-concept-0bf01.png["A Mule Palette with a search bar, options to add modules, a favorites section, and a core section listing modules and their types"]
...
image::index-4cdd4.png["A form with fields to configure a listener"]
...
image::index-0b4a5.png["A console log that displays status information for a Mule application"]

=== Problems

The Problems view displays a list of all issues encountered in your project.

image::index-7bcca.png["A table with two message flow errors"]
...
You can configure Anypoint Studio to use FIPS-compliant cryptographic libraries.  
This configuration is required for GovCloud customers and any environment that must comply with FIPS 140-2 standards.

This page explains how to enable FIPS mode in Studio and how to run Mule applications with FIPS-compliant settings.

=== Prerequisites

To use FIPS mode in Anypoint Studio, ensure the following:
...
. Navigate to *Preferences*. Click on *Anypoint Studio* and then *Runtimes*.
. Select the runtime that you want to configure.
. Select the checkbox *Enable FIPS support in Mule runtime*.

Studio automatically configures all required FIPS components. 

When you enable FIPS mode, it performs the following:
